...
int row, col; //accesorry vairables
...
//timing virables
unsigned int Gy521_ReadsPerSecond = 50;
...
/*
int ledCount;    //How many Leds will be on form 1-64
int perLedDelay; //how long to keep each LED on inside a frame (Display_WriteInterval / ledCount)
*/
...
int writeToDisplay() {
  ledsIndex = map((long)dXYZ, 0, 8192, 0, 63);
  ...
  */
  ...
  for (int i = 0; i <= ledsIndex; i++) {
    ...
    lc.setLed(0, row, col, true); /*delayMicroseconds( /*(Display_WriteInterval*1000)/128) 1000); */
  }
}
...
void setup() {
  ...
  lc.setIntensity(0,15); /* Set brightness to X/15 */
